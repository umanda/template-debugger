...
import { loadObject, updateObjectBounds, updateObjectEraser, updateObjectLock, updateObjectShadow } from "./fabric"
...
class ObjectImporter {
  ...
  public image(props: ImportOptions) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const element = new fabric.Image(image, {
          ...baseOptions,
          // @ts-ignore
          src,
          cropX: cropX || 0,
        ...
        updateObjectLock(element, props.item)
        updateObjectShadow(element, item.shadow)
        updateObjectEraser(element, item.eraser)
  ...
  public getBaseOptions(props: ImportOptions): Required<ILayer> {
    ...
    const { top, left, preview, id, name, type, eraser, erasable } = props.item as Required<ILayer>
