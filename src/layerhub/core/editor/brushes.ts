...
fabric.BaseBrush.prototype.convertToImg = function() {
  var pixelRatio = this.canvas.getRetinaScaling(),
    ...
    _vt = this.canvas.viewportTransform
...
export const SpraypaintBrush = fabric.util.createClass(fabric.BaseBrush, {
  ...
  onMouseDown: function(pointer) {
    ...
    this._vt = this.canvas.viewportTransform
    this.canvas.contextTop.globalAlpha = this.opacity
  ...
  onMouseMove: function(pointer) {
    this._lastPoint = this._point
    this._point = new fabric.Point(this._vt[4] + pointer.x * this._zoom, this._vt[5] + pointer.y * this._zoom)
  },
...
export const MarkerBrush = fabric.util.createClass(fabric.BaseBrush, {
  ...
  _render: function(pointer) {
    ...
    for (i = 0, len = this._size / this._lineWidth / 2; i < len; i++) {
      ...
      ctx.moveTo(
        this._vt[4] + this._lastPoint.x * this._zoom + lineWidthDiff,
        this._vt[5] + this._lastPoint.y * this._zoom + lineWidthDiff
      )
      ctx.lineTo(
        this._vt[4] + pointer.x * this._zoom + lineWidthDiff,
        this._vt[5] + pointer.y * this._zoom + lineWidthDiff
      )
      ctx.stroke()
    }
...
export const RibbonBrush = fabric.util.createClass(fabric.BaseBrush, {
  ...
  update: function() {
    ...
    for (var i = 0; i < painters.length; i++) {
      ctx.beginPath()
      ctx.moveTo(this._vt[4] + painters[i].dx * this._zoom, this._vt[5] + painters[i].dy * this._zoom)
      painters[i].dx -= painters[i].ax =
        (painters[i].ax + (painters[i].dx - this._lastPoint.x) * painters[i].div) * painters[i].ease
      painters[i].dy -= painters[i].ay =
        (painters[i].ay + (painters[i].dy - this._lastPoint.y) * painters[i].div) * painters[i].ease
      ctx.lineTo(this._vt[4] + painters[i].dx * this._zoom, this._vt[5] + painters[i].dy * this._zoom)
      ctx.stroke()
    }
  },
